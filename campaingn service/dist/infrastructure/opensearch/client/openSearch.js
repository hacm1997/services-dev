...
class OpenSearch {
    ...
    async inicializedClient() {
        ...
        const clientOp = new opensearch_1.Client(options);
        return clientOp;
    }
    async createIndexes() {
        const indexes = constant_1.OPS_INDEX;
        const client = this.inicializedClient();
        for (const indexName of indexes) {
            await (await client).indices
                .create({
                index: indexName,
                body: this.getIndexSettings(indexName),
            })
                .then((response) => {
                console.log(...oo_oo(`896435563_63_10_63_68_4`, `Index '${indexName}' created`, response.body));
            })
                .catch((error) => {
                console.log(...oo_oo(`896435563_66_10_66_67_4`, `Error creating index '${indexName}'`, error));
            });
        }
    }
    ...
    async indexEntity(entity, indexName) {
        return await (await this.initClient()).index({
            index: indexName,
            body: entity,
            id: entity.id,
            refresh: true,
    ...
    async reindex(sourceIndex, destinationIndex) {
        ...
        try {
            ...
            console.log(...oo_oo(`896435563_165_6_167_7_4`, `Re-indexación de ${sourceIndex} a ${destinationIndex} completada.`));
        }
        catch (error) {
    ...
    async searchByName(indexName, searchTerm, tenantId) {
        try {
            const client = await this.initClient();
            ...
            console.log(...oo_oo(`896435563_207_6_209_7_4`, `Exception occurred while searching by name in index ${indexName}: ${error}`));
            return {
                httpCode: 500,
                error: error,
            };
    ...
    async getByName(indexName, input) {
        const body = {
            query: {
                multi_match: {
                    query: input,
        ...
        try {
            const res = await (await this.initClient()).search({
                index: indexName,
                body,
            });
            ...
            console.log(...oo_oo(`896435563_252_6_252_63_4`, `Exception occurred while doing : ${error})`));
            return {
                httpCode: 500,
                data: [],
                error: error,
    ...
    async getByIndex(indexName, tenantId) {
        try {
            const res = await (await this.initClient()).search({
                index: indexName,
                body: {
            ...
            console.log(...oo_oo(`896435563_295_6_297_7_4`, `Exception occurred while searching index ${indexName}: ${error})`));
            return {
                httpCode: 500,
                data: [],
                error: error,
    ...
    async deleteByIndexIdAndTenant(indexName, id, tenantId) {
        try {
            await (await this.initClient()).delete({
                index: indexName,
                id: id,
                refresh: true,
                routing: tenantId,
            ...
            console.log(...oo_oo(`896435563_326_6_328_7_4`, `Exception occurred while deleting document from index ${indexName}: ${error}`));
            return {
                httpCode: 500,
                error: error,
            };
