...
let ContactRepository = ContactRepository_1 = class ContactRepository {
    ...
    async getContactByEmailTracking(email, tenantId) {
        const params = {
            TableName: Constants_1.TABLE_CONTACT_NAME,
            IndexName: Constants_1.REVERSE_INDEX,
            KeyConditionExpression: 'sid = :sid  AND begins_with(pid, :pid)',
    ...
    async getAllContacts() {
        const params = {
            ...
            KeyConditionExpression: 'sid = :sid AND begins_with(pid, :pid)',
            ExpressionAttributeValues: {
                ':pid': { S: ContactRepository_1.GROUP },
                ':sid': { S: microlib_1.DomainSession.getTenantId() },
            },
        };
        try {
            const data = await this.fullClientDynamodb.fullClient.query(params);
            console.log(...oo_oo(`2254513719_96_6_96_41_4`, 'data => ', data.Items));
    ...
    async putContactByTypeContact(item) {
    ...
    async putContact(id, contact) {
        const params = new lib_dynamodb_1.UpdateCommand({
            ...
            UpdateExpression: 'SET #full_name = :full_name, #email = :email, #phone = :phone, #contactStatus = :contactStatus, #age = :age, #lastComunication = :lastComunication, #personDetails = :personDetails, #companyDetails = :companyDetails',
            ExpressionAttributeNames: {
                '#full_name': 'full_name',
                '#email': 'email',
                '#phone': 'phone',
    ...
    async putContactCompany(id, company) {
        const params = new lib_dynamodb_1.UpdateCommand({
            ...
            UpdateExpression: 'SET #companyId = :companyId, #companyDetails = :companyDetails',
            ExpressionAttributeNames: {
                '#companyId': 'companyId',
                '#companyDetails': 'companyDetails',
            },
    ...
    async putContactPerson(id, person) {
        const params = new lib_dynamodb_1.UpdateCommand({
            ...
            UpdateExpression: 'SET #personId = :personId, #personDetails = :personDetails',
            ExpressionAttributeNames: {
                '#personId': 'personId',
                '#personDetails': 'personDetails',
            },
    ...
    createNewItemFromDomainModel(contact) {
        ...
        const pidsid = microlib_1.TUID.generateCreztuIDDynamoDBBasedOnTenant(ContactRepository_1.GROUP);
...
ContactRepository.GROUP = 'CNT';
exports.ContactRepository = ContactRepository = ContactRepository_1 = __decorate([
